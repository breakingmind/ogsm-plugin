...
4. Read `../../references/google-calendar-brief.md`.
5. Read `../../references/schedule-normalization.md`.
6. Produce the normalized schedule table using the exact schema before handoff.
7. If calendar event JSON is saved locally, run `node ../../scripts/normalize-calendar-events.js <calendar-events.json>`.
8. If the user asks to audit Google Calendar directly, first produce the normalized brief here, then hand off to `ogsm-audit-schedule` to score alignment.
9. Hand off to `ogsm-audit-schedule` when the user wants alignment review; do not score alignment in this skill.
...
- Do not read `../../references/google-calendar-brief.md` unless calendar connector or calendar event JSON input is involved.

## Tools

- May use Google Calendar connector to read events.
- May run `normalize-calendar-events.js` when calendar event JSON has been saved locally.
